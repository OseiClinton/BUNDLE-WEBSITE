...
const NETWORKS:Network[] = [
  { id:"mtn", name:"MTN", dot:"#f5b800", bundles:[
    {id:1,size:"1GB",validity:"1 Day",price:5,wholesale:4,popular:false},
    {id:2,size:"2GB",validity:"3 Days",price:9,wholesale:7.5,popular:false},
    {id:3,size:"5GB",validity:"7 Days",price:20,wholesale:17,popular:true},
    {id:4,size:"10GB",validity:"30 Days",price:38,wholesale:32,popular:false},
    {id:5,size:"20GB",validity:"30 Days",price:70,wholesale:60,popular:false},
    {id:6,size:"50GB",validity:"30 Days",price:160,wholesale:138,popular:false},
  ]},
  { id:"telecel", name:"Telecel", dot:"#ff1a27", bundles:[
    {id:1,size:"1GB",validity:"1 Day",price:5,wholesale:4,popular:false},
    {id:2,size:"3GB",validity:"3 Days",price:12,wholesale:10,popular:false},
    {id:3,size:"5GB",validity:"7 Days",price:20,wholesale:17,popular:true},
    {id:4,size:"10GB",validity:"30 Days",price:36,wholesale:30,popular:false},
    {id:5,size:"25GB",validity:"30 Days",price:80,wholesale:68,popular:false},
    {id:6,size:"50GB",validity:"30 Days",price:150,wholesale:128,popular:false},
  ]},
  { id:"airteltigo", name:"AirtelTigo", dot:"#ff4d38", bundles:[
    {id:1,size:"1GB",validity:"1 Day",price:4.5,wholesale:3.8,popular:false},
    {id:2,size:"2.5GB",validity:"3 Days",price:10,wholesale:8.5,popular:false},
    {id:3,size:"5GB",validity:"7 Days",price:19,wholesale:16,popular:true},
    {id:4,size:"10GB",validity:"30 Days",price:35,wholesale:29,popular:false},
    {id:5,size:"20GB",validity:"30 Days",price:65,wholesale:55,popular:false},
    {id:6,size:"40GB",validity:"30 Days",price:120,wholesale:102,popular:false},
  ]},
];

const STEPS = [
  {icon:"📡",step:"01",title:"Pick Network",desc:"Choose MTN, Telecel, or AirtelTigo."},
  {icon:"📦",step:"02",title:"Select Bundle",desc:"Pick your data size and validity."},
  {icon:"💳",step:"03",title:"Pay Instantly",desc:"MoMo or card via Paystack — secure."},
  {icon:"⚡",step:"04",title:"Receive Bundle",desc:"Delivered to your number in seconds."},
];
...
const FAQS = [
  ...
  {q:"Do I need an account to buy?",a:"No. You can checkout as a guest. Creating an account is optional."},
  ...
  {q:"What if my bundle doesn't arrive?",a:"Contact us immediately on WhatsApp or call our support line. We resolve all delivery issues within 15 minutes."},
];
...
export default function App() {
  ...
  const openBuy = (bundle:Bundle) => { setCart({...bundle, networkName:network.name}); setPhone(""); setRecipient(""); setPayMethod("momo"); setStep(1); setModal("buy"); };
  const handlePay = () => { if(!phone||phone.replace(/\s/g,"").length<9) return; setStep(2); setTimeout(()=>setModal("buySuccess"),2200); };
    ...
    <div style={{fontFamily:"'Barlow Condensed','Arial Narrow',sans-serif",background:"#fff",overflowX:"hidden"}}>
      ...
      <section id="hero" className="hero">
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div className="badge"><span className="bdot"/>NOW OPEN — SHOP DATA BUNDLES</div>
          <h1 className="h1">
          ...
          <p className="hsub">Fast, affordable data bundles for MTN, Telecel & AirtelTigo. Pay with MoMo or card. Delivered in seconds.</p>
          <div className="hbtns">
          ...
          <div className="hstats">
            ...
            <div><div className="snum">&lt;30s</div><div className="slbl">Delivery</div></div>
      ...
      <section id="bundles" className="sec sec-dark">
        ...
        <div className="ssub light">Choose your network and pick the perfect plan</div>
        ...
        <div className="bgrid">
          {network.bundles.map((b:Bundle)=>(
            <div key={b.id} className={`bcard ${b.popular?"bcard-pop":""}`}>
              ...
              <div className="bprice">{price(b)}<span className="bpsub"> GH₵</span></div>
              {isWholesale&&<div className="bsave">💰 Save GH₵{(b.price-b.wholesale).toFixed(1)}</div>}
        ...
        <div className="fgrid">
          <div>
            <div className="flogo">
              ...
              <div className="flogo-txt"><b>UNI</b><span>MARKET</span></div>
            </div>
          ...
          <div>
            ...
            <ul className="flinks">
              {["About UniMarket","Careers","Press"].map(l=><li key={l}>{l}</li>)}
            </ul>
          </div>
          <div>
            ...
            <ul className="flinks">
              ...
              <li onClick={()=>setModal("wholesale")} style={{cursor:"pointer"}}>Reseller Program</li>
            </ul>
          </div>
        </div>
        <div className="fbot">
          ...
          <div className="modal">
            ...
            {step===1?(
              <>
                ...
                <div className="msub">Buying {cart.size} on {cart.networkName}</div>
                <div className="obox">
                ...
                <div className="igrp">
                  ...
                  <input className="ifield" placeholder="e.g. 024 000 0000" value={phone} onChange={e=>setPhone(e.target.value)}/>
                </div>
                <div className="igrp">
                  <label className="ilbl">📲 Recipient Number (optional)</label>
                  <input className="ifield" placeholder="Leave blank to send to your number" value={recipient} onChange={e=>setRecipient(e.target.value)}/>
                </div>
                <div className="igrp">
                  <label className="ilbl">💳 Payment Method</label>
                  <div className="payopts">
                    <div className={`popt ${payMethod==="momo"?"on":""}`} onClick={()=>setPayMethod("momo")}>📱 MoMo</div>
                    <div className={`popt ${payMethod==="card"?"on":""}`} onClick={()=>setPayMethod("card")}>💳 Card</div>
                  </div>
                </div>
                <button className="btnpay" onClick={handlePay} disabled={!phone||phone.replace(/\s/g,"").length<9}>
      ...
      {modal==="buySuccess"&&(
        <div className="overlay" onClick={(e)=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal" style={{textAlign:"center"}}>
            ...
            <div className="sdesc2">Your bundle has been delivered. SMS confirmation coming to <strong>{phone}</strong>.</div>
            <button className="btnpay" onClick={()=>{setModal(null);setCart(null);}}>Done ✓</button>
          </div>
        </div>
      )}
            ...
            {!wSuccess?(
              <>
                ...
                <div style={{background:"#fff8f4",border:"1px solid #ffd5be",borderRadius:12,padding:"14px 18px",marginBottom:22,display:"flex",gap:12}}>
                  ...
                  <div style={{fontSize:13,color:"#666",fontFamily:"'Barlow',sans-serif",lineHeight:1.6}}>After registration, your account will be reviewed within <strong>24 hours</strong>. You'll get an SMS once verified.</div>
                </div>
                <div className="igrp">
              ...
              <>
                ...
                <div className="sdesc2">Thank you, <strong>{wForm.name}</strong>! We'll approve your account within <strong>24 hours</strong>. SMS confirmation to <strong>{wForm.phone}</strong>.</div>
                <button className="btnpay" onClick={()=>{setModal(null);setWSuccess(false);setWForm({name:"",phone:"",email:"",network:"mtn"});}}>Back to UniMarket</button>
              </>
            )}
          </div>
        </div>
